...
| `agent/invocation.ts` | The shared Soroban pipeline every contract call goes through: build → simulate → sign auth entries → assemble → sign envelope → submit → poll for a terminal status. Also error mapping (`contractError`, `diagnosticText`) and `getLatestLedger`, since both exist to serve this pipeline. |
...
| `agent/decoding.ts` | The inverse direction: a decoded contract struct → the SDK's public `*Info` types (`toAgentInfo`, `toChannelInfo`, `toJobInfo`, `toRateLimitStatus`), plus the field-level validators (`asString`, `asBigInt`, `asNumber`, `jobStatus`, `spendPeriod`, …) that turn a shape mismatch into a `StellarAgentError` instead of a `TypeError` three frames later. |
